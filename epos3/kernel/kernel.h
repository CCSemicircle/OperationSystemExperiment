...
#define NZERO 20             /*静态优先级的取值范围[-NZERO, NZERO-1]，值越小优先级越高*/
    int         nice;        //线程的静态优先级

#define PRI_USER_MIN    0
#define PRI_USER_MAX  127

    int priority;       // 线程的动态优先级，动态优先级的值越大优先级越高，且调度器以动态优先级为准
...
int sys_getpriority(int tid);           // 声明 获取优先级 函数
int sys_setpriority(int tid, int prio);    // 声明 设置优先级 函数
