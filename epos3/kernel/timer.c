...
{
  int nready = 0; // 就绪状态的线程个数,task0除外
  ...
  if (g_task_running != NULL)
  {
    ...
    {
      // 每次定时器中断，正在运行进程的estcpu加一,task0除外
      if (g_task_running != task0)
      {
        g_task_running->estcpu = fixedpt_add(g_task_running->estcpu, FIXEDPT_ONE);
      }
      // 每秒钟更新一次g_load_avg以及所有线程的estcpu
      if (g_timer_ticks % HZ == 0)
      {
        while (tsk != NULL)
        {
          if (tsk->state == TASK_STATE_READY && tsk != task0)
            nready++;
          fixedpt double_g_load_avg = fixedpt_mul(FIXEDPT_TWO, g_load_avg);
          fixedpt temp = fixedpt_div(fixedpt_mul(tsk->estcpu, double_g_load_avg), fixedpt_add(double_g_load_avg, FIXEDPT_ONE));
          tsk->estcpu = fixedpt_add(temp, fixedpt_fromint(tsk->nice));
          tsk = tsk->next;
        }
        fixedpt fraction_59_60 = fixedpt_div(fixedpt_fromint(59), fixedpt_fromint(60));
        fixedpt fraction_1_60 = fixedpt_div(FIXEDPT_ONE, fixedpt_fromint(60));
        g_load_avg = fixedpt_add(fixedpt_mul(fraction_59_60,g_load_avg),fixedpt_mul(fraction_1_60,fixedpt_fromint(nready)));
      }
