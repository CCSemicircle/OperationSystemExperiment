...
time_t sys_time() {
	time_t _time;
	_time = g_startup_time+g_timer_ticks / HZ;
	return _time;
}
...
static struct segment_descriptor gdt[NR_GDT] = {
    {// GSEL_NULL
     0x0,
     0x0,
     0x0,
...
extern idt_handler_t
    IDT_INTERRUPT(00),
    IDT_INTERRUPT(01), IDT_INTERRUPT(02), IDT_INTERRUPT(03),
...
void syscall(struct context *ctx)
{
    ...
    {
        int tid = *(int *)(ctx->esp + 4); // ctx->esp+4为第一个参数地址
        ctx->eax = sys_getpriority(tid);  // 调用系统调用实现函数
        break;
    }
